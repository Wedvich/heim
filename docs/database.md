...
**Encryption model:** 2-tier MEK → DEK with ML-KEM-768 (NIST FIPS 203) key encapsulation. Each principal has one DEK (stored here, wrapped using a shared secret derived from the MEK key pair). The MEK is an ML-KEM-768 asymmetric key pair — the public key is used to encapsulate (wrap) DEKs, and the private key is used to decapsulate (unwrap) them. This provides privilege separation: services that only create DEKs need only the public key. The `EMAIL_HMAC_KEY` used for email hashing is a separate, independent secret. A per-tenant KEK tier was considered but rejected — principals span tenants, so mapping DEKs to tenant-scoped KEKs would require multiple DEKs per principal.

The MEK secret key MUST be stored in a KMS (AWS KMS, GCP Cloud KMS, or HashiCorp Vault) in production. The application never holds the MEK secret key in memory — it sends the encrypted DEK to the KMS for unwrapping. For local development (Docker Compose), env var fallback is acceptable, but the application code must use the same KMS abstraction interface.

The MEK key pair and `EMAIL_HMAC_KEY` MUST be separate secrets with independent access controls. A single compromise should not expose both the encryption hierarchy and the identity correlation mechanism.

**DEK wrapping format:** For each DEK, `encrypted_key` stores `kemCiphertext || aesWrappedDek` — the 1088-byte ML-KEM-768 ciphertext concatenated with the AES-256-GCM wrapped DEK (~60 bytes). Total ~1148 bytes per row.

### Required secrets

| Secret             | Format                   | Purpose                                                                                                                                                                                                                                                       |
| ------------------ | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MEK_PUBLIC_KEY`   | ML-KEM-768 SPKI, base64  | MEK public key — used to encapsulate (wrap) per-principal DEKs. Safe to distribute to services that only create DEKs.                                                                                                                                         |
| `MEK_SECRET_KEY`   | ML-KEM-768 PKCS8, base64 | MEK secret key — used to decapsulate (unwrap) DEKs. Must be backed by KMS in prod.                                                                                                                                                                            |
...
All secrets MUST be independent. Rotation of one must not affect the others. In local development, random values generated at first start are acceptable (use `generate-mlkem-keypair.ts` for the MEK pair). In production, use env vars sourced from a secrets manager.

**Key loss:** If the MEK secret key is lost, all forgettable payloads become permanently unreadable. This is identical to the crypto shredding outcome but unintentional. The MEK secret key must have a documented backup/escrow procedure. This procedure should be specified in operational documentation before production deployment.

**MEK rotation:** generate a new ML-KEM-768 key pair, then re-wrap each DEK: decapsulate with the old secret key (identified by `mek_version`), encapsulate with the new public key, and update `mek_version`. Progress is trackable via `WHERE mek_version = <old>`.
