...
Each package should have a `CLAUDE.md` that describes:
...
### Recap Agent (planned — `.claude/agents/recap.md`)
...
### Review Agent (planned — `.claude/agents/review.md`)
...
- **Domain logic lives in `@heim/domain`** — aggregates, fold functions, and shared types. `@heim/api` and `@heim/web` are infrastructure/UI shells. `@heim/domain` is a dependency of both.
